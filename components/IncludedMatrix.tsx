  ...
  return (
    <div className="max-w-4xl mx-auto py-12 px-4">
      <div className="text-center mb-10">
        <h2 className="text-3xl font-bold text-[#1B2B3A]">The 90/10 Split</h2>
        <p className="text-gray-500">Know exactly where your membership ends and your insurance begins.</p>
      </div>
      <div className="grid grid-cols-3 gap-4 border-t border-gray-100 pt-6">
        <div className="text-sm font-bold uppercase text-gray-400">Medical Service</div>
        <div className="text-sm font-bold uppercase text-teal-600 text-center">DPC (The 90%)</div>
        <div className="text-sm font-bold uppercase text-[#1B2B3A] text-center">Insurance (The 10%)</div>
        
        {coverage.map((row) => (
          <div key={row.service} className="contents">
            <div className="py-4 border-b border-gray-50 text-gray-700 font-medium">{row.service}</div>
            <div className="py-4 border-b border-gray-50 flex justify-center">
              {row.dpc ? <Check className="text-teal-500" /> : <span className="text-gray-200">-</span>}
            </div>
            <div className="py-4 border-b border-gray-50 flex justify-center">
              {row.insurance ? <ShieldCheck className="text-[#1B2B3A]" /> : <span className="text-gray-200">-</span>}
            </div>
          </div>
        ))}
      </div>
      <div className="mt-8 bg-amber-50 p-4 rounded-2xl flex gap-3 items-center border border-amber-100">
